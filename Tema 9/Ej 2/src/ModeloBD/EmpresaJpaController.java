    ...
    public void create(Empresa empresa) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            for (Persona personaListPersona : empresa.getPersonaList()) {
                Empresa oldIdempresaOfPersonaListPersona = personaListPersona.getIdempresa();
                personaListPersona.setIdempresa(empresa);
                personaListPersona = em.merge(personaListPersona);
                if (oldIdempresaOfPersonaListPersona != null) {
                    oldIdempresaOfPersonaListPersona.getPersonaList().remove(personaListPersona);
                    oldIdempresaOfPersonaListPersona = em.merge(oldIdempresaOfPersonaListPersona);
                }
            }
        ...
        try {
            ...
            for (Persona personaListOldPersona : personaListOld) {
                if (!personaListNew.contains(personaListOldPersona)) {
                    ...
                    illegalOrphanMessages.add("You must retain Persona " + personaListOldPersona + " since its idempresa field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Persona personaListNewPersona : personaListNew) {
                if (!personaListOld.contains(personaListNewPersona)) {
                    Empresa oldIdempresaOfPersonaListNewPersona = personaListNewPersona.getIdempresa();
                    personaListNewPersona.setIdempresa(empresa);
                    personaListNewPersona = em.merge(personaListNewPersona);
                    if (oldIdempresaOfPersonaListNewPersona != null && !oldIdempresaOfPersonaListNewPersona.equals(empresa)) {
                        oldIdempresaOfPersonaListNewPersona.getPersonaList().remove(personaListNewPersona);
                        oldIdempresaOfPersonaListNewPersona = em.merge(oldIdempresaOfPersonaListNewPersona);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            for (Persona personaListOrphanCheckPersona : personaListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Empresa (" + empresa + ") cannot be destroyed since the Persona " + personaListOrphanCheckPersona + " in its personaList field has a non-nullable idempresa field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
